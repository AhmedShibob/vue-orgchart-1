(function(global, factory) {
  ...
  var axios = require("../../../src/axiosBase");
  var Swal = require("sweetalert2/dist/sweetalert2.js");
  ...
  var OrgChart$1 = (function() {
    function OrgChart(options) {
      ...
      if (
        opts.exportButton &&
        !chartContainer.querySelector(".oc-export-btn")
      ) {
        ...
        opts.exportButtonName === "Export"
          ? (exportBtn.innerHTML = "Export")
          : (exportBtn.innerHTML = "" + opts.exportButtonName);
    ...
    createClass(OrgChart, [
      ...
      },
      {
        ...
        value: function _getDescElements(ancestors, selector) {
          ...
          ancestors.forEach(function(el) {
            return results.push.apply(
              results,
              toConsumableArray(el.querySelectorAll(selector))
            );
          });
          return results;
        }
      ...
      },
      {
        ...
        value: function _attachRel(data, flags) {
          ...
          if (data.children) {
            ...
            try {
              for (
                var _iterator = data.children[Symbol.iterator](), _step;
      ...
      },
      {
        ...
        value: function _isInAction(node) {
          return (
            node.querySelector(":scope > .edge").className.indexOf("fa-") > -1
          );
        }
      ...
      },
      {
        ...
        value: function _getNodeState(node, relation) {
          ...
          if (relation === "parent") {
            ...
            if (
              state.exist &&
              this._isVisible(criteria.parentNode.children[0])
            ) {
              state.visible = true;
            }
      ...
      },
      {
        ...
        value: function _hoverNode(event) {
          ...
          if (event.type === "mouseenter") {
            ...
            Array.from(node.querySelectorAll(":scope > .edge")).forEach(
              function(el) {
                el.classList.remove(
                  "fa-chevron-up",
                  "fa-chevron-down",
      ...
      },
      {
        ...
        value: function hideSiblings(node, direction) {
          ...
          try {
            for (
              var _iterator2 = animatedNodes[Symbol.iterator](), _step2;
          ...
          this._one(
            ...
            function(event) {
              ...
              var someLines = temp.slice(
                1,
                direction ? sibs.length * 2 + 1 : -1
              );
      ...
      },
      {
        ...
        value: function _startLoading(arrow, node) {
          ...
          if (
            typeof chart.dataset.inAjax !== "undefined" &&
          ...
          var exportBtn = this.chartContainer.querySelector(
            ".oc-export-btn" +
              (opts.chartClass !== "" ? "." + opts.chartClass : "")
          );
      ...
      },
      {
        ...
        value: function hideChildren(node) {
          ...
          var descendants = Array.from(
              lastItem.querySelectorAll(".node")
            ).filter(function(el) {
          ...
          if (!isVerticalDesc) {
            descendants.forEach(function(desc) {
              Array.prototype.push.apply(
                lines,
                that._prevAll(
                  that._closest(desc, function(el) {
                    return el.classList.contains("nodes");
                  }),
                  ".lines"
                )
        ...
        value: function showChildren(node) {
          ...
          if (
            ...
            temp.forEach(function(el) {
              Array.prototype.push.apply(
                descendants,
                Array.from(el.querySelectorAll(".node")).filter(function(el) {
                  return that._isVisible(el);
                })
              );
            });
      ...
      },
      {
        ...
        value: function _clickBottomEdge(event) {
          ...
          if (childrenState.exist) {
            ...
            if (
              Array.from(temp.querySelectorAll(".node")).some(function(node) {
                return (
                  _this8._isVisible(node) && node.classList.contains("slide")
                );
              })
            ) {
              return;
      ...
      {
        key: "_complementLine",
        value: function _complementLine(
          oneSibling,
          siblingCount,
          existingSibligCount
        ) {
        ...
        value: function _buildSiblingNode(nodeChart, nodeData, callback) {
          ...
          var that = this,
            ...
            existingSibligCount =
              nodeChart.parentNode.nodeName === "TD"
                ? this._closest(nodeChart, function(el) {
                    return el.nodeName === "TR";
                  }).children.length
                : 1,
          ...
          if (nodeChart.parentNode.nodeName === "TD") {
            ...
            that._buildChildNode.call(
              that,
              that._closest(nodeChart.parentNode, function(el) {
                return el.nodeName === "TABLE";
              }),
              nodeData,
              function() {
                if (++childCount === newSiblingCount) {
                  var siblingTds = Array.from(
                    that._closest(nodeChart.parentNode, function(el) {
                      return el.nodeName === "TABLE";
                    }).lastChild.children
                  );
      ...
      },
      {
        ...
        value: function _clickHorizontalEdge(event) {
          ...
          if (siblingsState.exist) {
            ...
            if (
              siblings.some(function(el) {
                var node = el.querySelector(".node");

                return (
                  _this10._isVisible(node) && node.classList.contains("slide")
                );
              })
            ) {
              return;
      ...
      },
      {
        ...
        value: function _onDragStart(event) {
          ...
          let willDrag = JSON.parse(nodeDiv.dataset.source);
          sNodeInput.dataset.node = willDrag._id;
          ...
          Array.from(this.chart.querySelectorAll(".node")).forEach(function(
            node
          ) {
      ...
      },
      {
        ...
        value: function _onDrop(event) {
          ...
          let sNodeInput = document.getElementById("selected-node");
          console.log(sNodeInput.dataset.node);
          let draggedon = JSON.parse(dropZone.dataset.source);
          console.log(draggedon._id);
          var selfe = this;

          axios.default
            .put(`organizations/${sNodeInput.dataset.node}/move`, {
              parentId: draggedon._id
            })
            .then(response => {
              console.log(response);
              console.log(thisFromvoEdit);
              document.getElementsByClassName("orgchart")[1].remove();
              document.getElementById("selected-node-ar").innerHTML = "";
              document.getElementById("selected-node-en").innerHTML = "";
              thisFromvoEdit.getAlldata();
              Toast.fire({
                type: "success",
                title: "تم تحريك الوحده بنجاح"
              });
            })
            .catch(error => {
              //console.log("faild API KB");
              console.log(error);
              Toast.fire({
                type: "error",
                title: "لا يمكن تحريك هذه الوحده"
              });
              document.getElementsByClassName("orgchart")[1].remove();
          ...
          if (!dropZone.parentNode.parentNode.nextElementSibling) {
            ...
            if (dropSibs.length === 1) {
              ...
              _rightEdge.setAttribute(
                "class",
                "edge horizontalEdge rightEdge fa"
              );
              dropSibs[0].appendChild(_rightEdge);
              _leftEdge.setAttribute(
                "class",
                "edge horizontalEdge leftEdge fa"
              );
          ...
          if (dragColSpan > 2) {
            ...
            var _temp4 =
              dragZone.parentNode.nextElementSibling.nextElementSibling;

            _temp4.children[1].remove();
            _temp4.children[1].remove();

            var dragSibs = Array.from(
        ...
        value: function _createNode(nodeData, level) {
          ...
          return new Promise(function(resolve, reject) {
            if (nodeData.children) {
              ...
              try {
                for (
                  var _iterator3 = nodeData.children[Symbol.iterator](), _step3;
            ...
            nodeDiv.innerHTML =
              '\n        <div class="title">' +
              nodeData[opts.nodeTitle].en +
              "<br/>" +
              nodeData[opts.nodeTitle].ar +
              "</div>\n        " +
              (opts.nodeContent
                ? '<div class="content">' +
            ...
            if (opts.toggleCollapse) {
              ...
              nodeDiv.addEventListener(
                "click",
                that._dispatchClickEvent.bind(that)
              );
            }
            if (opts.draggable) {
              nodeDiv.addEventListener(
                "dragstart",
                that._onDragStart.bind(that)
              );
      ...
      },
      {
        ...
        value: function buildHierarchy(appendTo, nodeData, level, callback) {
          ...
          var that = this,
            ...
            isVerticalNode =
              opts.verticalDepth && level + 1 >= opts.verticalDepth;
      ...
      },
      {
        ...
        value: function _clickExportButton() {
          var opts = this.options,
            ...
            sourceChart = chartContainer.querySelector(
              ".orgchart:not(.hidden)"
            ),
          ...
          window
            .html2canvas(sourceChart, {
              ...
              onclone: function onclone(cloneDoc) {
                var canvasContainer = cloneDoc.querySelector(
                  ".canvasContainer"
                );
            ...
            .then(function(canvas) {
              var downloadBtn = chartContainer.querySelector(
                ".oc-download-btn"
              );
      ...
      },
      {
        ...
        value: function _onPanStart(event) {
          ...
          if (
            this._closest(event.target, function(el) {
              return el.classList && el.classList.contains("node");
            }) ||
      ...
      },
      {
        ...
        value: function _onTouchEnd(event) {
          ...
          if (chart.dataset.pinching) {
            ...
            var diff =
              chart.dataset.pinchDistEnd - chart.dataset.pinchDistStart;
  ...
  var bindEventHandler = function bindEventHandler(
    selector,
    type,
    fn,
    parentSelector
  ...
  var clickNode = function clickNode(event) {
    ...
    document.getElementById("selected-node-ar").innerHTML = JSON.parse(
      sNode.dataset.source
    ).name.ar;
    document.getElementById("selected-node-en").innerHTML = JSON.parse(
      sNode.dataset.source
    ).name.en;
    sNodeInput.value = sNode.querySelector(".title").textContent;
    var myElement = JSON.parse(
      sNode.querySelector(".title").parentElement.dataset.source
    );
    console.log(myElement._id);
    sNode.id = myElement._id;
    sNodeInput.dataset.node = myElement._id;
  };
